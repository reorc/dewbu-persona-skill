...
voc evidence search --pain-points <specific_tag> --source amazon_review
```
...
# Step 3: 看痛点分布
voc evidence search --occupations hunter --pain-points "" --limit 50
# 或者用 stats
...
```bash
# Amazon 评论中的反馈
voc evidence search --query <keyword> --source amazon_review --limit 10

# 邮件中的反馈
voc evidence search --query <keyword> --source email --limit 10

# 或者用 SQL 做更灵活的统计
voc sql "SELECT source_type, count(*) FROM evidence_index WHERE EXISTS (SELECT 1 FROM unnest(pain_points_mapped) t WHERE t ILIKE '%battery%') GROUP BY source_type"
```
...
```bash
# 低星评论的痛点
voc evidence search --query <keyword> --star-min 1 --star-max 2 --limit 20

# 高星评论的优势
voc evidence search --query <keyword> --star-min 4 --limit 20
```
...
```bash
# 先搜索找到相关 evidence
voc evidence search --query <keyword> --limit 5
...
当预定义命令不够灵活时，用 `voc sql` 执行任意 SELECT：
...
# 高消费用户的评论
voc sql "SELECT e.title, e.content_snippet, u.total_spend FROM evidence_index e JOIN user_profiles u USING(user_id) WHERE u.total_spend > 500 AND e.source_type = 'amazon_review' ORDER BY u.total_spend DESC LIMIT 10"

# 邮件平台分布
voc sql "SELECT platform, count(*) FROM email_signals GROUP BY platform"
