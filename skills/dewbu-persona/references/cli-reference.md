...
配置保存在 `~/.dewbu/config.json`。先在对应部署的 webapp `Admin` -> `Accounts` -> `More` -> `API keys` 里生成 API key（管理员账号可管理画像，普通用户为只读），再写入 CLI：

```bash
dewbu config set --svc-base-url https://<deployment-domain>/ --api-key dewbu_live_xxx
dewbu config show
```
